...
public interface PostRepository extends JpaRepository<Post, UUID> {
    ...
    @Query("SELECT new io.github.DoubleBerserker.stele.dto.PostDto(p.id, p.title, SUBSTRING(p.content, 1, 200)) " +
            "FROM Post p ORDER BY p.createdAt DESC ")
    List<PostDto> findLatestPostsByNumberOfPosts(Pageable pageable);
